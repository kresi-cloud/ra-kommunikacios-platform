...
Feladatok, események/naptár, értesítési outbox és a hozzájuk tartozó
felületi adatlekérés továbbra is a Supabase-en fut (lásd 8. fejezet).
...
| `private.is_project_owner()` | `isProjectOwnerOfRecord()` + `isProjectOwner()` | Előbbi: `projects.ownerUserId` egyezik; utóbbi ezen felül `project_owner` szerepet is megkövetel (global vagy az adott projektre) |
...
(`server/routes/projects.ts` `errorResponse()`).

**Tesztlefedettség:** `server/authz.test.ts` (7 teszt: aktív/inaktív fiók,
global vs. projekt-scope szerep, visszavont szerep, technikai admin
tiltása) valós, memóriabeli SQLite ellen.
...
### 5.3. Egyéb végpontok (`server/index.ts`)
...
| Feladatok, naptár projekt-legördülője | **Átkötve** csak a projektlistázás | `src/pages/TasksPage.tsx`, `src/pages/CalendarPage.tsx` (`listProjects()` hívás) |
| Feladatok, események, naptár (adat), értesítések | **Nincs átkötve** | továbbra is `src/lib/supabase.ts` kliens |
...
azonosítás lekerült a Supabase-ről, a feladat-/esemény-/értesítésoldalak
mostantól RLS-hibát kapnak (nincs Supabase-munkamenet), és az általános
hibabannereket mutatják (pl. "A feladatok betöltése nem sikerült.") – ez a
portolásukig várt, dokumentált állapot, nem tényleges hiba.

A `ProjectsPage` létrehozó űrlapján a gazda kijelölése egyelőre önmagára
korlátozott (a bejelentkezett felhasználó), mert a felhasználólistázó
végpont (`list_assignable_users` megfelelője) még nincs portolva.
...
4. **`GET /api/projects` teljesítménye:** soronkénti jogosultság-ellenőrzés,
   nem SQL-szintű szűrés – sok projektnél újragondolandó.
5. **E2E-tesztek háttérszervere:** a Playwright `webServer` konfigurációja
...
6. **Kötelező MFA:** a `roles.requiresMfa` mező adatként létezik (pl.
...
Az itt leírt szelet (azonosítás + projektmodul) akkor tekinthető késznek,
ha:
