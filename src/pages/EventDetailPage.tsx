...
import { getCalendarEvent, inviteEventUser, listActiveUsers, listEventParticipants, respondToEvent } from '../api/events'
...
export function EventDetailPage() {
  ...
  const users = useQuery({ queryKey: ['active-users'], queryFn: () => { if (!supabase) throw new Error(); return listActiveUsers(supabase) }, enabled: Boolean(supabase && event.data?.responsibleUserId === session?.user.id) })
